...
class Scanloader(torch.utils.data.Dataset):
    ...
    def split_dataset(self):
        train_size = int(0.7 * self.length)
        valid_size = int(0.2 * self.length)
        train_data, valid_data, infer_data = torch.utils.data.random_split(self, [train_size, valid_size, self.length - train_size - valid_size])
